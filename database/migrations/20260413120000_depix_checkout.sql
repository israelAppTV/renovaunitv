-- DePix: identificador de checkout e RPC de fulfillment agnóstica ao provedor.

ALTER TABLE public.orders
  ADD COLUMN IF NOT EXISTS depix_checkout_id text;

CREATE UNIQUE INDEX IF NOT EXISTS idx_orders_depix_checkout_id
  ON public.orders (depix_checkout_id)
  WHERE depix_checkout_id IS NOT NULL;
...
BEGIN
  ...
  VALUES ('depix', p_idempotency_key, p_order_id)
  ...
  VALUES (
    ...
    v_code_id,
    'email',
    jsonb_build_object('provider', 'depix', 'payment_id', p_payment_id)
  );
...
REVOKE ALL ON FUNCTION public.fulfill_order_payment(uuid, text, text, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.fulfill_order_payment(uuid, text, text, text) TO service_role;
